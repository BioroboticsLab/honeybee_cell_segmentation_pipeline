...
class HexLatticeGraph:
    ...
    def _in_bounds(self, pos) -> bool:
        ...
        # Check if position is within comb mask
        if self.config.comb_mask is not None:
            x_int, y_int = int(round(x)), int(round(y))
            if 0 <= y_int < self.config.comb_mask.shape[0] and 0 <= x_int < self.config.comb_mask.shape[1]:
                return self.config.comb_mask[y_int, x_int] > 0
            return False
