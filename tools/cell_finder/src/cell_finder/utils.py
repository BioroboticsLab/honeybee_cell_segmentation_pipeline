...
def show_cells_on_image(color_image, found_cells):
    ...
    cells_on_image = draw_detected_cells(img_copy, found_cells)
...
def print_radius_distribution(results: dict):
    all_radii = []
    for data in results.values():
        # data: (img_path, duration, num_cells, color_img, matches)
        matches = data[-1]
        all_radii.extend([r for _, _, r, _ in matches])
    if not all_radii:
        print("No detected cells to analyze.")
        return
    all_radii = np.array(all_radii)
    print(
        f"Radius stats: min={all_radii.min():.2f}, max={all_radii.max():.2f}, mean={all_radii.mean():.2f}, std={all_radii.std():.2f}"
    )
    plt.figure(figsize=(8, 4))
    plt.hist(all_radii, bins=30, color="skyblue", edgecolor="black")
    plt.title("Distribution of detected cell radii")
