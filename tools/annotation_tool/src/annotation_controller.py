...
class AnnotationController:
    def __init__(self, data_dir: Path, config_path: Path | None = None) -> None:
        ...
        self.label_map = self._load_label_map(config_path)
        self.loader = DataLoader(data_dir, self.label_map, self.logger)
    ...
    def _load_label_map(self, config_path: Path | None = None) -> dict[str, str]:
        path = config_path if config_path is not None else LABELS_PATH
        try:
            with open(path) as f:
                label_data = json.load(f)
        except Exception as e:
            self.logger.error(f"Could not open label config file: {path}\nError: {e}")
            print(f"ERROR: Could not open label config file: {path}\n{e}")
            exit(1)
...
def main():
    ...
    parser.add_argument("--config", type=Path, default=None, help="Optional path to label config file (JSON)")
    ...
    controller = AnnotationController(data_dir, config_path)
    controller.run()
