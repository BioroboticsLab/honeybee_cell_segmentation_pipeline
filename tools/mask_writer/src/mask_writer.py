...
import cv2
import numpy as np
from mask_utils import visualize_mask_by_index

# LABELS_PATH = Path(__file__).parents[1] / "data" / "label_classes_consolidated.json"


class MaskWriter:
    def __init__(
        self, input_img_path: Path, input_json_path: Path, label_classes_path: Path, mask_out_path: Path, visualize_indices: list[int] = None
    ) -> None:
        ...
        self.label_classes_file = label_classes_path / "label_classes.json"
        assert self.label_classes_file.is_file()
    ...
    def _load_label_map(self) -> list[dict[str, str]]:
        ...
        with open(self.label_classes_file) as f:
    ...
    def run(self) -> None:
        ...
        for idx, (name, (img_path, json_path)) in enumerate(files):
            ...
            if self.visualize_indices is not None and idx in self.visualize_indices:
                visualize_mask_by_index(self.masks_out_path, idx, self._label_map)
...
def main():
    ...
    parser.add_argument("--label_classes_path", type=str, default=None, help="Optional path to the label map. Defaults to input")
    parser.add_argument(
    ...
    label_classes_path = Path(args.label_classes_path) if args.label_classes_path else input_path
    masks_output_path = Path(args.masks_output_path) if args.masks_output_path else input_path

    mask_creator = MaskWriter(input_path, input_json_path, label_classes_path, masks_output_path, args.visualize_indices)
    mask_creator.run()
